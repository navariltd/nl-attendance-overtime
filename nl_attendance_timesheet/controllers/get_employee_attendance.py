...
def get_employee_attendance(employee_id, start_date, end_date):
    ...
    include_early_entry = frappe.db.get_single_value(
        SETTINGS_DOCTYPE, "include_early_entry"
    )
    ...
    if not include_early_entry:
        for entry in attendance_records:
            if entry.in_time:
                in_time_str = str(entry.in_time).split(".")[
                    0
                ]  # split because entry.in_time sometimes comes in this format -> 2024-02-15 08:01:48.858030
                in_time = datetime.strptime(in_time_str, "%Y-%m-%d %H:%M:%S").time()
                shift_start_time = datetime.strptime(
                    str(entry.shift_start_time), "%H:%M:%S"
                ).time()
                shift_end_time = datetime.strptime(
                    str(entry.shift_end_time), "%H:%M:%S"
                ).time()

                if in_time < shift_start_time:
                    extra_hours = (
                        (shift_start_time.hour - in_time.hour)
                ...
                total_shift_hours = (
                    (shift_end_time.hour - shift_start_time.hour)
...
def get_employee_overtime_attendance(employee, start_date, end_date):
    ...
    conditions = [
        ...
        timesheet.start_date[start_date:end_date],
    ]
    ...
    query = (
        ...
        .where(Criterion.all(conditions))
        .orderby(timesheet.start_date, order=Order.asc)
    )
